...
void process_map(const map_type& partial_map, map_type& global_map, std::atomic<std::uintmax_t>& directory_counter) {
    ...
    for (const auto& [ext, info] : partial_map) {
        global_map[ext].num_files += info.num_files;
        global_map[ext].total_size += info.total_size;
    }
...
// Function to generate a partial map from a chunk of paths
map_type generate_map(const std::vector<fs::path>& chunk) {
    map_type res;
    for (const fs::path& pth : chunk) {
        if (fs::is_directory(pth)) {
            auto dir_name = pth.filename().string(); // Corrected to use filename instead of stem
